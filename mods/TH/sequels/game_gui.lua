-- chunkname: @./sequels/game_gui.lua
...
function TowerMenuButton:initialize(item, entity)
	local scale_factor = V.v(0.9, 0.9)
	...
	if item.check and IS_MOBILE then
		...
		image_check.scale = scale_factor
	...
	elseif item.action == "upgrade_power" or item.action == "tw_free_action" then
		...
		image_frame.propagate_on_click = true
		image_frame.scale = scale_factor
	...
	if not image_halo and item.halo then
		...
		image_halo.pos = get_pos(image_halo)
	end

	if image_halo then
		image_halo.propagate_on_click = true
		image_halo.hidden = true
		image_halo.scale = scale_factor
		image_halo.pos = get_pos(image_halo)
	...
	if image_frame then
		image_frame.scale = scale_factor
		image_frame.pos = get_pos(image_frame)
	...
	self.image_highlight.scale = scale_factor
