-- chunkname: @./sequels/kviews_screen_map_sequels.lua
...
function TowerRoomView:initialize(size, image_name, base_scale)
	...
	-- 清除存档内额外增加的防御塔选择，避免坏档
	if #user_data.towers.selected - 5 > 0 then
		for i = 6, #user_data.towers.selected do
			log.warning("Removing extra tower from selected: %s", user_data.towers.selected[i])
			
			user_data.towers.selected[i] = nil
		end
...
function ItemRoomView:initialize(size, image_name, base_scale)
	...
	function button_buy.on_click(this)
		...
		if user_data.gems < info.item_cost then
			...
			if not PS.services.iap or not PS.services.iap:is_premium() then
